...
var eliminarDuplicadosLista = function(listado) {
    var listaSinDuplicados = listado.filter(encontrarDuplicadosLista);
    return listaSinDuplicados;
}

var encontrarDuplicadosLista = function(elem, index, self){
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    //Se recorre el array de restaurantes y se va agregando al array creado, todas las listaUbicaciones o ciudades encontradas
    this.restaurantes.map(function(rest){
        listaUbicaciones.push(rest.ubicacion);
    });
    // for (var i = 0; i < this.restaurantes.length; i++) {
    //     listaUbicaciones.push(this.restaurantes[i].ubicacion);
    // }
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ubicacionesSinDuplicados = eliminarDuplicadosLista(listaUbicaciones);

    return ubicacionesSinDuplicados.sort();
}
...
Listado.prototype.obtenerRubros = function() {
    var listaRubros = [];
    this.restaurantes.map(function(rest){
        listaRubros.push(rest.ubicacion);
    });
    // for (var i = 0; i < this.restaurantes.length; i++) {
    //     listaRubros.push(this.restaurantes[i].rubro);
    // }

    var rubrosSinDuplicados = eliminarDuplicadosLista(listaRubros)
    ;

    return rubrosSinDuplicados.sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    return horariosSinDuplicados.sort();
}
